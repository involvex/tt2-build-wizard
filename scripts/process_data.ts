...
function extractObject(content, variableName) {
    ...
    let depth = 0;
    let endIdx = -1;
    for (let i = startIdx + `const ${variableName} = `.length; i < content.length; i++) {
        if (content[i] === '{') depth++;
        if (content[i] === '}') {
            depth--;
            if (depth === 0) {
    ...
    const jsonStr = content.substring(startIdx + `const ${variableName} = `.length, endIdx);
...
writeFileSync('packages/logic/src/reductions.json', JSON.stringify(reductions, null, 2));
