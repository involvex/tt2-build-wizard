...
export class ArtifactOptimizerEngine {
	...
	public getCostAtLevel(artifactId: string, level: number): number {
		...
		if (!art) return 0
		return (
			(art.CostCoef / (1 + art.CostExpo)) * Math.pow(level, 1 + art.CostExpo)
		)
	}

	public getCostToLevel(
		artifactId: string,
		currentLevel: number,
		targetLevel: number,
	): number {
		return (
			this.getCostAtLevel(artifactId, targetLevel) -
			this.getCostAtLevel(artifactId, currentLevel)
		)
	}

	public getLevelForRelics(
		artifactId: string,
		currentLevel: number,
		relics: number,
	): number {
		...
		const currentCostSum =
			(art.CostCoef / costExpoIncreased) *
			Math.pow(currentLevel, costExpoIncreased)
	...
	private calculateEfficiency(
		...
		const nextLevel = this.getLevelForRelics(
			artifactId,
			currentLevel,
			relicsToSpend,
		)
	...
	public optimize(
		...
		while (remainingRelics > 0 && iterations < maxIterations) {
			...
			const targetLevel = this.getLevelForRelics(
				bestArtId,
				workingLevels[bestArtId],
				stepRelics,
			)
			const cost = this.getCostToLevel(
				bestArtId,
				workingLevels[bestArtId],
				targetLevel,
			)

			if (cost > remainingRelics || targetLevel <= workingLevels[bestArtId])
				break
