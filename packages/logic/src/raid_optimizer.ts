	...
	public suggestDecks(
		...
		if (sortedCards.length >= 3) {
			topDecks.push({
				...
				totalPower: sortedCards
					.slice(0, 3)
					.reduce(
						(sum, id) => sum + this.getCardPower(id, playerCardLevels[id]),
						0,
					),
			})
		}
		...
		const afflictions = sortedCards.filter(
			id => this.cards[id].Category === 'Affliction',
		)
