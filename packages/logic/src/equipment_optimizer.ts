	...
	public getRecommendations(
		...
		for (const setId in this.sets) {
			...
			if (build === 'Shadow Clone') {
				if (setId === 'Skulls') {
					priority += 100
					reason = 'Essential for Shadow Clone splash.'
				}
				if (setId === 'RuthlessNecromancer') {
					priority += 90
					reason = 'Primary damage and utility boost.'
				}
			}

			if (build === 'Clan Ship') {
				if (setId === 'AncientWarrior') {
					priority += 100
					reason = 'Doubles companion attack rate.'
				}
				if (setId === 'AirshipEngineer') {
					priority += 80
					reason = 'Significant Clan Ship damage boost.'
				}
			}
