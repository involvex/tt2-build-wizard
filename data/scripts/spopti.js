...
const MAX_SKILL_LEVEL = Math.max(
	...Object.values(skillInfo).map(skill => skill.MaxLevel),
)
...
class Player {
	...
	effArray() {
		...
		for (const talentID in skillInfo) {
			...
			if (
				playerskills[skill]['Selection'] === false ||
				(this.MaxStage && this.MaxStage < stageReq)
			) {
			...
			for (let j = currLevel; j < maxLevel; j++) {
				...
				const reduction =
					reductions[talentID][this.typeDamage] +
					reductions[talentID][this.typeGold] * this.goldWeight
	...
	_calcEff(
		...
		switch (id) {
			...
			case 'StreamOfBladesMultiCastSkill':
				...
				curr =
					((10 * (currA ?? 1)) ** (!currB ? 0 : currB + multicast)) **
					reductionFactor
			...
			case 'TwilightGatheringMultiCastSkill':
				next =
					((10 * nextA) ** (nextB + multicast)) ** reductionFactor *
					nextC ** reductionFactor
				curr =
					((10 * (currA ?? 1)) ** (!currB ? 0 : currB + multicast)) **
						reductionFactor *
					(currC || 1) ** reductionFactor
				efficiency = next / curr
				break
			...
			case 'PetBonusBoost': // Ember Arts
				...
				curr =
					(currA || 1) ** reductionFactor * (currB || 1) ** (reduction_2 / cost)
				efficiency = next / curr
				break
			...
			case 'TerrifyingPact': // Terrifying Pact
				const focoActive =
					playerskills['Forbidden Contract']['Selection'] === true ||
					playerskills['Forbidden Contract']['Level'] >= 1
				const rocoActive =
					playerskills['Royal Contract']['Selection'] === true ||
					playerskills['Royal Contract']['Level'] >= 1
				...
				curr =
					(currA || 1) ** focoReduction * (currB || 1) ** (rocoReduction / cost)
				efficiency = next / curr
				break
			...
			case 'HandOfMidasMultiCastSkillBoost': // Midas Overflow
				...
				next =
					(mc_bonus_A[nextB + multicast] * nextA ** (nextB + multicast)) **
						reductionFactor *
					mc_bonus_B[nextB + multicast] ** ((gold_2 * goldWeight) / cost)
				curr =
					(mc_bonus_A[!currB ? 0 : currB + multicast] *
						(currA ?? 1) ** (!currB ? 0 : currB + multicast)) **
						reductionFactor *
					mc_bonus_B[!currB ? 0 : currB + multicast] **
						((gold_2 * goldWeight) / cost)
				efficiency = next / curr
	...
	_spellDamage(spells, extraLevels) {
		...
		for (const k of spells) {
			...
			const reduction =
				spell_reductions[k][this.typeDamage] +
				spell_reductions[k][this.typeGold] * this.goldWeight
	...
	get totalEffect() {
		...
		let result =
			(10 ** (total - Math.floor(total))).toFixed(2) + 'e' + Math.floor(total)
...
class Optimize {
    ...
    */
		...
		const c = Array(8)
			.fill()
			.map(() => new Array(resultLength).fill(-100000))
		const d = Array(8)
			.fill()
			.map(() => new Array(resultLength).fill(-100000))
	...
	static optTree(efficiencies, currLevels, SP) {
		...
		for (let i = 0; i < goodRows; ++i) {
			...
			while (
				maxLevInc < MID_POINT &&
				optData[i][maxLevInc + MID_POINT] <
					optData[i][maxLevInc + MID_POINT + 1]
			)
				++maxLevInc

			const newArray = new Array(optData[i][maxLevInc + MID_POINT] + 1).fill(
				-100000,
			)
			...
			tree.push(
				this.mergeEff(newArray, tree[tree.length - 1][0], SPReq, goodMasks),
			)
		}
		trees.push(tree)
		...
		for (let i = 0; i < trees.length; ++i) {
			...
			prefTrees.push(
				this.mergeTrees(trees[i][trees[i].length - 1][0], oldVal, totSP),
			)
		}
		...
		const mode =
			document
				.querySelector('.optimizeMode')
				.getAttribute('data-maxCumlEff') === 'true'
		if (mode) {
			const effVals = this.intoOneFastc(
				prefTrees[trees.length][0],
				standardMasks,
			)[0]
		...
		for (let i = trees.length; i >= 1; --i) {
			const vaalFast = this.intoOneFastd(
				prefTrees[i][0],
				prefTrees[i][1],
				standardMasks,
				totSP,
			...
			for (let j = trees[i - 1].length - 1; j >= 1; --j) {
				const curValFast = this.intoOneFastd(
					trees[i - 1][j][0],
					trees[i - 1][j][1],
					usedMasks,
					totSPinTree,
				...
				let levHere = 0
				while (
					levHere < optData[0].length &&
					optData[goodRows - levels.length - 1][levHere + MID_POINT] !== SPHere
				) {
					++levHere
				}
		...
		for (let i = 0; i < goodRows; ++i) {
			if (oldLevels[i].length === 0) {
				...
				newLevels.push([
					Number(levels[goodRows - i - 1]) + Number(oldLevels[i][0]),
				])
			}
		}
...
class PageHelper {
	static findSkillInput(talentId) {
		...
		const treeNameElement = document.querySelector(
			`.trees .grid-item.treeName#${branch}`,
		)
	...
	static getMaxLevels() {
		// get max skill levels from skillInfo
		let max = []
	...
	static setMinMax() {
		// apply min and max values for each skill
	...
	static checkValue(input) {
		// check if skill evel is valid within bounds
	...
	static setBuildName() {
		// set name of build in the skill tree
	...
	static resetTree() {
		// reset skill levels in the tree
	...
	static treeSP() {
		// obtain total SP for each tree and apply to tree
		...
		document.querySelector('#buildSP').innerHTML =
			`${sumTotal} / ${playerSP} (${remainingSP} SP remaining)`

		return treeTotals
	}

	static spUsed() {
		// obtain total SP used
		let optData = player.skillArray
		let sumTotal = 0
	...
	static toTree(levels) {
		// take array of skill levels and apply them to the tree
	...
	static saveSkillInfo() {
		// get info for each skill and save to playerInfo
	...
	static currSkillInfo() {
		// obtain info for each skill
		const currLevels = this.processSkills(input => [input.value], [0])
		const currSelections = this.processSkills(
			input => [input.getAttribute('data-select') === 'true'],
			[false],
		)
		const currLocks = this.processSkills(
			input => [input.getAttribute('data-lock') === 'true'],
			[false],
		)
	...
	static buildExport() {
		// take current skill levels and create the import string
		let importString = {
	...
	static setTree() {
		// apply tree sp and import string
		this.treeSP()
		this.buildExport()
	}
	...
	static checkboxCheck(element) {
		// handle locking and selecting of skills
	...
	static toggleMode(element) {
		// toggle between optimizing modes
	...
	static doAll(element) {
		// lock/unlock/select/deselect all skills
	...
	static totalEffect() {
		// get total effect of build and apply to tree
	...
	static baselineQol() {
		...
		const skillIdToIndex = Object.fromEntries(
			skillIds.map((id, index) => [id, index]),
		)
		...
		for (const [i, {Name: skillName}] of Object.entries(skillInfo)) {
			...
			const value = !playerskills[skillName].Selection
				? curr_level
				: Math.max(level, curr_level)
	...
	static isBaselineValid(baselineLevels, currLevels) {
		...
		for (const i in baselineLevels) {
			...
			const spCost =
				getCumulativeSP[i][baselineLevel] - getCumulativeSP[i][currLevel]
	...
	static importSave(string) {
		// handle importing of player data or build export
	...
	static copyToClipboard() {
		// copy build export to clipboard
		$('exportString').select()
	...
	static expand(button, elementId) {
		// expands hidden fields from button
	...
	static builderSave(element) {
		...
		const buildSP = document
			.querySelector('#buildSP')
			.innerHTML.match(/^\d+(?=\s*\/)/)[0]
		const defaultName = `${buildType}, ${buildSP} SP`
		const buildName =
			document.querySelector('#buildSaveName').value || defaultName
		const oldName =
			document.querySelector('#buildSaveMode').selectedOptions[0].text
		let buildNum
	...
	static renameBuild(oldName, newName, buildNum) {
		...
		const newData = JSON.parse(
			JSON.stringify(saves).replace(`"${oldName}":`, `"${newName}":`),
		)
		playersaves = newData
		buildSaveMode.querySelector(`option[value="${buildNum + 1}"]`).text =
			newName
	}
		...
		for (const action in actions) {
			if (element.classList.contains(action)) {
				for (const key in actions) {
					...
					if (
						key === action &&
						currentElement.getAttribute('data-toggle') === 'true'
					) {
    ...
    */
	...
	static updateBrightness() {
		const gridItemsWithInput = document.querySelectorAll(
			'.trees .grid-item input',
		)
		...
		gridItemsWithInput.forEach(item => {
			...
			let newBG
			if (item.value > 0) {
				newBG = backgroundImage.replace(
					'rgba(37, 37, 37, 0.6), rgba(37, 37, 37, 0.6)',
					'rgba(37, 37, 37, 0.1), rgba(37, 37, 37, 0.1)',
				)
...
function letterToSci(coef) {
	let [base, magnitude] = parseFloat(coef)
		.toExponential()
		.split('e')
		.map(parseFloat)
...
function optimize() {
	...
	PageHelper.toTree(
		Optimize.optTree(
			player.skillArray,
			player.currLevels,
			Number(playerstats['SkillPoints']),
		),
	)
...
function setEventListeners() {
	document.querySelector('.calculate').addEventListener('click', () => {
		...
		let treeSP = PageHelper.treeSP().reduce(
